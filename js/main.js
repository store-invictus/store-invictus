...
(function () {
  ...
  /* ────────────────────────────────────────────
     ...
     Validação real + integração Formspree (configurável)
     Troque xykdnwrw pelo seu ID em formspree.io
  ──────────────────────────────────────────── */
  const FORMSPREE_ENDPOINT = 'https://formspree.io/f/xykdnwrw';
  ...
  if (contactForm) {
    ...
    contactForm.addEventListener('submit', async (e) => {
      ...
      // Se Formspree não estiver configurado, simula
      if (FORMSPREE_ENDPOINT.includes('xykdnwrw')) {
        await simulateDelay(1200);
        showFormMsg('✅ Mensagem enviada com sucesso! Responderei em breve.', 'sucesso');
        contactForm.reset();
        resetBtn();
        return;
      }
      ...
      try {
        ...
        if (res.ok) {
          ...
          showFormMsg('❌ Erro ao enviar. Tente pelo WhatsApp.', 'erro');
        }
      } catch {
        showFormMsg('❌ Sem conexão. Tente pelo WhatsApp.', 'erro');
      } finally {
        resetBtn();
      }
